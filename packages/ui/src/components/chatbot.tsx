...
function statusColor(status: Status): string {
    switch (status) {
        case "loading":    return "#EF9F27";
        case "ready":      return "#1D9E75";
        case "generating": return "#378ADD";
...
function statusLabel(status: Status, progress: number): string {
    switch (status) {
        case "loading":    return `Loading model… ${progress}%`;
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            try {
                ...
                if (cancelled) { engine.unload(); return; }
                engineRef.current = engine;
                setStatus("ready");
                setProgress(100);
                addMessage("assistant", `Hi ${context.user.name}! I'm here to help you navigate iBank. What can I help you with? [DISCLAIMER: This website has been created for WPI's CS 3733 Software Engineering as a class project and is not in use by Hanover Insurance.]`);
            } catch (err) {
                if (!cancelled) {
...
${appGuide ? `\n## App guide\n${appGuide}` : ""}\``;
    ...
    const sendMessage = useCallback(async () => {
        ...
        const chatMessages: webllm.ChatCompletionMessageParam[] = [
            ...
            ...messages.map(m => ({ role: m.role, content: m.content } as webllm.ChatCompletionMessageParam)),
        ...
        <div style={styles.root}>
                ...
                <div style={styles.panel}>
                    ...
                    <div style={styles.header}>
                        ...
                        <div style={{
                            ...
                            animation: status === "loading" || status === "generating" ? "pulse 1s ease-in-out infinite" : "none",
                        }} />
                    </div>
                    ...
                    {!hasGpu && (
                        <div style={styles.gpuWarning}>
                            Your browser doesn't support WebGPU. The AI assistant requires Chrome 113+ or Edge 113+.
                        </div>
                    )}
                    ...
                    {status === "loading" && (
                        <div style={styles.loadPane}>
                            ...
                            <div style={styles.progressTrack}>
                                <div style={{ height: "100%", width: `${progress}%`, background: "#111", borderRadius: 2, transition: "width 0.3s" }} />
                            </div>
                    ...
                    <div ref={messagesRef} style={styles.messages}>
                        {messages.map(msg => (
                            <div key={msg.id} style={{
                                display: "flex", flexDirection: "column", maxWidth: "86%",
            ...
            <textarea
                ...
                style={{
                    ...
                    lineHeight: 1.5, minHeight: 36, maxHeight: 100,
                    outline: "none", opacity: isDisabled ? 0.4 : 1,
                }}
            />
                        <button
                            onClick={sendMessage}
